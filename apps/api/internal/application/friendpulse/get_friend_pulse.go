...
// Candidate is eligibility/ranking metadata for a Social Context that may contribute a friend-visible projection.
...
type ContextProjector interface {
	Project(ctx context.Context, viewerID domainidentity.ID, ownerID domainidentity.ID, socialContextID string) (domainprivacy.ContextProjection, bool, error)
}
...
func NewGetFriendPulse(friendships ActiveFriendshipReader, candidates CandidateReader, projector ContextProjector) GetFriendPulse {
	return GetFriendPulse{friendships: friendships, candidates: candidates, projector: projector, now: time.Now}
}
...
func (uc GetFriendPulse) Execute(ctx context.Context, query Query) (Pulse, error) {
	...
	if err != nil { return Pulse{}, err }
	friendID, err := domainidentity.NewID(query.FriendID)
	if err != nil { return Pulse{}, err }

	active, err := uc.friendships.IsActiveBetween(ctx, viewerID, friendID)
	if err != nil { return Pulse{}, err }
	if !active { return Pulse{}, ErrFriendPulseUnauthorized }

	candidates, err := uc.candidates.ListForOwner(ctx, friendID)
	if err != nil { return Pulse{}, err }

	now := time.Now()
	if uc.now != nil { now = uc.now() }
	projected := make([]projectedCandidate, 0, len(candidates))
	for _, candidate := range candidates {
		if candidate.RepetitionSuppressed || (!candidate.EligibleUntil.IsZero() && !candidate.EligibleUntil.After(now)) {
			continue
		}
		projection, visible, err := uc.projector.Project(ctx, viewerID, friendID, candidate.SocialContextID)
		if err != nil { return Pulse{}, err }
		if !visible { continue }
		projected = append(projected, projectedCandidate{projection: projection, signalScore: candidate.SignalScore, observedAt: candidate.ObservedAt, contextID: candidate.SocialContextID})
	}

	sort.SliceStable(projected, func(i, j int) bool {
		if projected[i].signalScore != projected[j].signalScore { return projected[i].signalScore > projected[j].signalScore }
		if !projected[i].observedAt.Equal(projected[j].observedAt) { return projected[i].observedAt.After(projected[j].observedAt) }
		return projected[i].contextID < projected[j].contextID
	})

	limit := len(projected)
	if limit > maxPulseItems { limit = maxPulseItems }
	pulse := Pulse{Items: make([]Item, 0, limit)}
	for _, candidate := range projected[:limit] { pulse.Items = append(pulse.Items, Item{Projection: candidate.projection}) }
	return pulse, nil
}
