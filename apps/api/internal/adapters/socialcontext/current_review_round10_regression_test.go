...
func TestDeterministicGeneratorDoesNotBindCompoundBareAbandonmentAcrossUnsupportedAntecedent(t *testing.T) {
	generator := NewDeterministicGenerator()
	got, err := generator.Generate(context.Background(), appsc.ContextGenerationInput{Activities: []appsc.ContextGenerationActivity{
		{
			ID:      "activity-compound",
			Content: "開始研究分散式系統，最近開始學習法文並且每天練習口說，後來放棄了",
		},
	}})
	if err != nil {
		t.Fatalf("Generate() error = %v", err)
	...
	if !strings.Contains(got.Meaning, "分散式系統") {
		t.Fatalf("Generate() meaning = %q, want unsupported local antecedent to block bare abandonment from retracting earlier local distributed-systems topic", got.Meaning)
	}
}
